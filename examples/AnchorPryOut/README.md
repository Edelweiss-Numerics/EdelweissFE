...
`mesh/` is generated; `zsymm_complete.txt` is generated by `make_zsymm_complete.py` and is **not**
optional — the mesh's own `z_symm_concrete` / `z_symm_steel` sets are incomplete, and running
without the complete set puts nodes on the symmetry plane under no constraint at all.
