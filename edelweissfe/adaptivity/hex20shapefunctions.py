...
"""Reference shape functions and topology data for the 20-node serendipity hexahedron (Marmot
C3D20). Pure math: no dependency on the AMR machinery (:mod:`~edelweissfe.adaptivity.refinement`,
:mod:`~edelweissfe.adaptivity.topologybase`) or on the live FEModel, so it can be read, tested and
reused (e.g. by a mesh generator) on its own. :class:`~edelweissfe.adaptivity.hex20topology.Hex20Topology`
is the thin :class:`~edelweissfe.adaptivity.topologybase.TopologyBase` adapter built on top of it.
...
def hex20_shape_grad(xi, eta, zeta):
    ...
    (:meth:`~edelweissfe.adaptivity.hex20topology.Hex20Topology.inverse_map`) so each iteration needs
    one evaluation instead of a value call plus a 3-point finite-difference Jacobian.
...
if __name__ == "__main__":
    ...
    print("hex20shapefunctions self-tests PASSED")
