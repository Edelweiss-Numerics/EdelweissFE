...
#  ---------------------------------------------------------------------
"""2:1 balancing decides face adjacency exactly (see ``AdaptiveMesh._faceDescriptors``): on a curved and
...
def _twoRoots(curvedWarped, splitFactor=2):
    ...
    return mesh, mesh.add_root(A, 0), mesh.add_root(B, 0)
...
def _childOfBTouchingA(mesh, b):
    ...
    return next(k for k in kids if mesh.elements[k]["referenceLow"][0] == mesh.elements[b]["referenceLow"][0])
...
def test_aTwoLevelJumpInsideOneRootIsBalanced():
    ...
    root = mesh.add_root(hex20_box_coords(0.0, 1.0, 0.0, 1.0, 0.0, 1.0), 0)
